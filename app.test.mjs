...
vm.runInContext(`${source}\nglobalThis.testApi = { CATALOG, buildRecommendations, categorizeItem, conversationQuantity, createDemoHistory, createEmptyStore, dedupeItems, findAmbiguousItem, getSubstitutes, itemKey, listCatalogEstimate, migrateStore, normalizeItemName, parseCatalogSearch, parseCommand, parseQuantity, productListItem, productVariants, quantityRemoval, searchCatalog, selectProductVariant, shoppingListText, shouldHandleFinalSpeech, speechLanguage, transcriptCorrection, validHistoryEvent, validateItemName, viewFromHash };`, context);
const { CATALOG, buildRecommendations, categorizeItem, conversationQuantity, createDemoHistory, createEmptyStore, dedupeItems, findAmbiguousItem, getSubstitutes, itemKey, listCatalogEstimate, migrateStore, normalizeItemName, parseCatalogSearch, parseCommand, parseQuantity, productListItem, productVariants, quantityRemoval, searchCatalog, selectProductVariant, shoppingListText, shouldHandleFinalSpeech, speechLanguage, transcriptCorrection, validHistoryEvent, validateItemName, viewFromHash } = context.testApi;
const plain = (value) => JSON.parse(JSON.stringify(value));
const DAY_MS = 86_400_000;

test("keeps every JavaScript ID binding present in the HTML shell", () => {
  const ids = [...source.matchAll(/querySelector\("#([^"\s]+)"\)/gu)].map((match) => match[1]);
  for (const id of ids) assert.match(html, new RegExp(`id=["']${id}["']`, "u"), `Missing #${id}`);
});

test("maps linked mobile views and totals selected variant prices in paise", () => {
  ...
  assert.deepEqual(plain(listCatalogEstimate([
    { name: "Bread", quantity: 2, unitPricePaise: 4500, productId: "bread-whole", completed: false },
    { name: "Unknown grocery", quantity: 3, completed: false },
    { name: "Milk", quantity: 1, unitPricePaise: 6800, completed: true }
  ])), { totalPaise: 9000, savingsPaise: 2000, priced: 1, unpriced: 1 });
});
...
test("assigns common items without substring false positives", () => {
  ...
  assert.equal(categorizeItem("2 bottles of milk"), "Dairy & Eggs");
  assert.equal(categorizeItem("toilet paper"), "Household");
  assert.equal(categorizeItem("green tea"), "Beverages");
  assert.equal(categorizeItem("दूध"), "Dairy & Eggs");
  assert.equal(categorizeItem("leche"), "Dairy & Eggs");
  assert.equal(categorizeItem("steak"), "Other");
});

test("sorts expanded grocery categories using specific phrases first", () => {
  assert.equal(categorizeItem("soy sauce"), "Grains & Staples");
  assert.equal(categorizeItem("frozen peas"), "Other");
  assert.equal(categorizeItem("salmon"), "Other");
...
test("does not merge equivalent products when their units differ", () => {
...
test("filters speech fillers and separates natural space-delimited groceries", () => {
  assert.deepEqual(plain(parseCommand("umm add onion spinach milk you know", "en-IN")).items, [
    { name: "Pyaaz", quantity: 1, unit: "item" },
    { name: "Spinach", quantity: 1, unit: "item" },
    { name: "Milk", quantity: 1, unit: "item" }
  ]);
  assert.deepEqual(plain(parseCommand("उम्म प्याज पालक दूध जोड़ो", "hi-IN")).items.map(({ name }) => name), ["Pyaaz", "पालक", "Milk"]);
  assert.deepEqual(plain(parseCommand("eh añade cebolla espinaca leche por favor", "es-ES")).items.map(({ name }) => name), ["Pyaaz", "Espinaca", "Milk"]);
});

test("parses Romanized Hindi aliases, implicit adds, connectors, and speech-style suffixes", () => {
  assert.equal(parseCommand("atta", "en-IN").items[0].name, "Flour");
  assert.equal(parseCommand("chini", "en-IN").items[0].name, "Sugar");
  assert.deepEqual(plain(parseCommand("atta doodh chini ad", "en-IN")).items.map(({ name }) => name), ["Flour", "Milk", "Sugar"]);
  assert.deepEqual(plain(parseCommand("atta doodh chini", "en-IN")).items.map(({ name }) => name), ["Flour", "Milk", "Sugar"]);
  assert.deepEqual(plain(parseCommand("atta aur doodh and chini", "en-IN")).items.map(({ name }) => name), ["Flour", "Milk", "Sugar"]);
  assert.deepEqual(plain(parseCommand("dhoodh", "en-IN")).items, [{ name: "Milk", quantity: 1, unit: "item" }]);
  assert.deepEqual(plain(parseCommand("list me atta doodh daal do", "en-IN")).items.map(({ name }) => name), ["Flour", "Milk"]);
  assert.deepEqual(plain(parseCommand("mujhe atta doodh chini chahiye", "en-IN")).items.map(({ name }) => name), ["Flour", "Milk", "Sugar"]);
  assert.equal(parseCommand("atta list mein daalo", "en-IN").items[0].name, "Flour");
  assert.equal(parseCommand("doodh le lena", "en-IN").items[0].name, "Milk");
  assert.equal(parseCommand("atta doodh hata do", "en-IN").intent, "remove");
  assert.equal(parseCommand("chini hata do", "en-IN").items[0].name, "Sugar");
});

test("parses Hinglish quantities before or after an item", () => {
  assert.deepEqual(plain(parseCommand("add 2 kilo atta", "en-IN")).items, [{ name: "Flour", quantity: 2, unit: "kg" }]);
  assert.deepEqual(plain(parseCommand("atta 2 kilo", "en-IN")).items, [{ name: "Flour", quantity: 2, unit: "kg" }]);
  assert.deepEqual(plain(parseCommand("2 packet dhoodh", "en-IN")).items, [{ name: "Milk", quantity: 2, unit: "pack" }]);
...
test("parses arbitrary Indian grocery names with quantities and units", () => {
  const cases = [
    ...
    ["2 kilo atta", { name: "Flour", quantity: 2, unit: "kg" }],
...
test("categorizes Indian staples while preserving Other for unmatched groceries", () => {
  for (const name of ["Bajra", "Jowar", "Ragi", "Atta", "Maida", "Suji", "Poha", "Besan", "Dal", "Rajma", "Chana", "Moong", "Masoor", "Urad", "Jeera", "Haldi", "Dhaniya", "Mustard seeds"]) assert.equal(categorizeItem(name), "Grains & Staples");
...
test("uses canonical alias identity for duplicates and list matching", () => {
  ...
  assert.equal(categorizeItem("dhoodh"), "Dairy & Eggs");
});
...
test("normalizes high-confidence Makki flour aliases into one canonical item", () => {
  ...
  assert.equal(parsed.every(({ name }) => categorizeItem(name) === "Grains & Staples"), true);
...
test("keeps multiword groceries whole and flags partially unknown speech", () => {
  ...
  assert.deepEqual(partial.items.map(({ name }) => name), ["Flour", "Sugar"]);
...
test("suggests deterministic grocery spelling corrections", () => {
  ...
  assert.deepEqual(items[0], { name: "Pyaaz", quantity: 1, unit: "item", correction: { from: "onoin", to: "onion" } });
...
test("parses Hindi add, multiple items, remove, and quantity commands", () => {
  ...
  assert.deepEqual(plain(parseCommand("दूध की मात्रा तीन करो", "hi-IN")).item, { name: "Milk", quantity: 3, unit: null });
...
test("parses Spanish add, multiple items, remove, and quantity commands", () => {
  ...
  assert.deepEqual(plain(parseCommand("Cambia la cantidad de leche a tres botellas", "es-ES")).item, { name: "Milk", quantity: 3, unit: "bottle" });
...
test("maps a search result into the existing list and history payload", () => {
  ...
  assert.deepEqual(plain(productListItem(product)), { name: "Colgate Total Toothpaste", canonicalProductId: "colgate total toothpaste", productId: "toothpaste-colgate", brand: "Colgate", size: "150 g", unitPricePaise: 18900, quantity: 1, unit: "pack", category: "Personal Care" });
...
test("creates a grouped plain-text list for sharing", () => {
  const text = shoppingListText([
    ...
    { name: "Milk", quantity: 2, unit: "pack", category: "Dairy & Eggs", completed: true }
  ]);
  assert.match(text, /Produce[\s\S]*• Onion — 1 item/u);
  assert.match(text, /Dairy[\s\S]*✓ Milk — 2 packs/u);
});

test("repairs 5x conservatively and never accepts it as an item name", () => {
  const parsed = plain(parseCommand("Add 5x", "en-IN"));
  assert.deepEqual(parsed.items, [{ name: "Eggs", quantity: 5, unit: "item" }]);
  assert.equal(parsed.confirmation.suggested, "Add 5 eggs");
  assert.match(validateItemName("5x"), /fragment/u);
  assert.equal(transcriptCorrection("buy 2x", "en").suggested, "Add 2 eggs");
});

test("executes a final speech result once and ignores repeats", () => {
  assert.equal(shouldHandleFinalSpeech(false, "Add milk"), true);
  assert.equal(shouldHandleFinalSpeech(true, "Add milk"), false);
  assert.equal(shouldHandleFinalSpeech(false, "   "), false);
});

test("normalizes multilingual sugar identity across list operations", () => {
  assert.equal(new Set(["sugar", "shakkar", "sakkar", "चीनी", "शक्कर"].map(itemKey)).size, 1);
  assert.equal(parseCommand("remove shakkar", "en-IN").items[0].name, "Sugar");
  assert.equal(parseCommand("increase चीनी by 2", "en-IN").item.name, "Sugar");
});

test("migrates old categories and aliases once without double-counting", () => {
  const old = { version: 1, list: { items: [
    { id: "a", name: "Sugar", quantity: 2, unit: "pack", category: "Pantry", completed: false },
    { id: "b", name: "शक्कर", quantity: 3, unit: "pack", category: "Grains / Pantry", completed: false },
    { id: "bad", name: "5x", quantity: 1, unit: "item", category: "Other", completed: false }
  ] }, history: [], preferences: {} };
  const once = plain(migrateStore(old));
  const twice = plain(migrateStore(once));
  assert.equal(once.list.items.length, 1);
  assert.equal(once.list.items[0].quantity, 5);
  assert.equal(once.list.items[0].category, "Grains & Staples");
  assert.deepEqual(twice, once);
});

test("parses completion, price ranges, exact sizes, and Hinglish quantities", () => {
  assert.deepEqual(plain(parseCommand("Mark milk as completed", "en-IN")), { ok: true, intent: "complete", language: "en", name: "Milk" });
  assert.deepEqual(plain(parseCommand("Doodh 3 packet kar do", "en-IN")).item, { name: "Milk", quantity: 3, unit: "pack" });
  assert.deepEqual(plain(parseCatalogSearch("rice between ₹100 and ₹500", "en")), { minPrice: 100, maxPrice: 500, query: "rice" });
  assert.deepEqual(plain(parseCatalogSearch("one litre milk", "en")), { exactSize: "1 l", query: "milk" });
});

test("selects catalog variants deterministically and calculates conversational quantities", () => {
  const variants = productVariants("sugar");
  assert.equal(selectProductVariant("the second one", variants).brand, "Tata");
  assert.equal(selectProductVariant("the cheapest one", variants).brand, "Trust");
  assert.equal(selectProductVariant("Tata wala chahiye", variants).brand, "Tata");
  assert.deepEqual(plain(conversationQuantity("two packets", "en")), { quantity: 2, unit: "pack" });
  assert.equal(new Set(variants.map(({ id }) => id)).size, variants.length);
});

test("accepts flexible English and Hinglish search word order", () => {
  const reordered = parseCommand("Under ₹200, show me Colgate toothpaste", "en-IN");
  assert.equal(reordered.intent, "search");
  assert.equal(reordered.filters.brand, "Colgate");
  assert.equal(reordered.filters.maxPrice, 200);
  assert.equal(parseCommand("Shakkar ke brands dikhao", "en-IN").filters.query, "sugar");
  const hinglish = parseCommand("200 rupaye ke andar toothpaste dikhao", "en-IN");
  assert.equal(hinglish.filters.maxPrice, 200);
  assert.equal(hinglish.filters.query, "toothpaste");
});
